...
char tester[] = "This is the second episode:\nThe Realm of Black Magic\n\nAncient castles and\nstrange beasts ahead...";

void word_break(char *str)
{
	int i,line;
	char *cur,*dst;

	scr_center_lines = 1;
	dst = &scr_centerstring[0];
	//cur = &tester[0];
	cur = str;
	line = i = 0;
	do
	{
		for(i=0;i<32;i++)
		{
			if(cur[i] == 0 || isspace(cur[i]) || ispunct(cur[i]) || cur[i] == '\n')
			{
				break;
			}
		}
...
{
	int l;
	if(1 || cl.intermission == 2 || cl.intermission == 3)
	{
		word_break(str);
	}
	else
...
void SCR_EraseCenterString (void)
{
	...
	scr_copytop = 1;
	for(row=0;row<scr_erase_lines;row++)
	{
		for(x=0;x<vid.width;x+=8)
		{
			Draw_CharacterCenter(x,y+(row*8),' ');
		}
	}
...
// the finale prints the characters one at a time
	...
	// scan the width of the line
		...
		for (j=0 ; j<l ; j++, x+=8)
		{
			Draw_CharacterCenter (x, y, start[j]);	
			if (!remaining--)
				return;
		}
...
// notify the refresh of the change
	R_ViewChanged (&r_refdef.vrect, sb_lines, vid.aspect);
}
...
// bound field of view
	...
	r_refdef.fov_x = scr_fov.value;
	r_refdef.fov_y = CalcFov (r_refdef.fov_x, r_refdef.vrect.width, r_refdef.vrect.height);
	Con_Printf("r_refdef.vrect: %3d %3d %3d %3d\nfov: %3d %3d\n",r_refdef.vrect.x,r_refdef.vrect.y,r_refdef.vrect.width,r_refdef.vrect.height,r_refdef.fov_x,r_refdef.fov_y);
...
void SCR_DrawFPS (void)
{
	...
	if (!scr_showfps.value)		// turn off for screenshots
		return;
	...
	x = vid.width - 64;
	for(p = &temp[0];*p;p++,x+=8)
		Draw_CharacterCenter (x, 0, *p);
}
...
// draw a fresh screen
	...
	do
	{
		...
		Sys_SendKeyEvents ();
				...
				key_lastpress != K_NDS_A &&
				key_lastpress != K_NDS_B);

	scr_fullupdate = 0;
	SCR_UpdateScreen ();

	return key_lastpress == 'y' || key_lastpress == K_NDS_A;
}
