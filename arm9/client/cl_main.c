...
// clear other arrays	
	...
	memset (cl_entities, 0, sizeof(cl_entities));
			//ent = (entity_t *)Hunk_AllocName(sizeof(entity_t),"entity");
...
void CL_PrintEntities_f (void)
{
	...
	for (i=0,e=cl_entities ; i<cl.num_entities ; i++,e++)
	{
		ent = *e;
		Con_Printf ("%3i:",i);
		if (!ent->model)
...
// start on the entity after the world
	for (i=1,e=cl_entities+1 ; i<cl.num_entities ; i++,e++)
	{
		ent = *e;
		if (!ent->model)
		{	// empty slot
...
// if the object wasn't included in the last packet, remove it
		...
		if (ent->model->flags)
		{
			...
			if		(ent->model->flags & EF_GRENADE)	R_RocketTrail (oldorg, ent->origin, 1);
			else if	(ent->model->flags & EF_GIB)		R_RocketTrail (oldorg, ent->origin, 2);
