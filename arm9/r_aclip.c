...
// copy vertexes and fix seam texture coordinates
	if (ptri->facesfront)
	{
		fv[0][0] = r_pfinalverts[ptri->vertindex[0]];
		fv[0][1] = r_pfinalverts[ptri->vertindex[1]];
		fv[0][2] = r_pfinalverts[ptri->vertindex[2]];
	}
	else
	{
		for (i=0 ; i<3 ; i++)
		{
			fv[0][i] = r_pfinalverts[ptri->vertindex[i]];
...
// clip
	...
	{
		for (i=0 ; i<3 ; i++)
			av[i] = r_pauxverts[ptri->vertindex[i]];
