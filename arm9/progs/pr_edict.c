...
char *ED_NewString (char *string)
{
	...
	l= strlen(string) + 1;
	str = (char *)Hash_Get(pr_strings_hash, string);
	if(str == 0)
	{
		bucket_t *bucket = (bucket_t*)Hunk_Alloc(sizeof(bucket_t) + l);
		str = (char *)(bucket+1);
		new_p = str;
		...
		Hash_Add(pr_strings_hash, str, str, bucket);
...
edict_t * NEXT_EDICT(edict_t *e)
{
	...
	//	Sys_Error ("NEXT_EDICT: bad pointer");
...
int	EDICT_TO_PROG(edict_t *e)
{
	...
	if (n < 0 || n >= sv.num_edicts)
		Sys_Error ("EDICT_TO_PROG: bad pointer");
//#define	EDICT_TO_PROG(e) ((byte *)e - (byte *)sv.edicts)

	prog = n * pr_edict_size;
...
{
	int n = prog / pr_edict_size;
	if (n < 0 || n >= sv.num_edicts)
		Sys_Error ("PROG_TO_EDICT: bad pointer");
...
eval_t *	PROG_TO_EVAL(int prog)
{
	...
	eval_t *v;
	int n = prog / pr_edict_size;
	int x = prog % pr_edict_size;
	if (n < 0 || n >= sv.num_edicts)
		Sys_Error ("PROG_TO_EVAL: bad pointer");
