...
void Sbar_DrawTransPic (int x, int y, qpic_t *pic)
{
	if (cl.gametype == GAME_DEATHMATCH)
		...
		Draw_TransPic (x + ((vid.conwidth - CON_SCREEN_WIDTH)>>1), y + (vid.conheight-SBAR_HEIGHT), pic);
}
...
void Sbar_DrawCharacter (int x, int y, int num)
{
	if (cl.gametype == GAME_DEATHMATCH)
		...
		Draw_Character ( x + ((vid.conwidth - CON_SCREEN_WIDTH)>>1) + 4 , y + vid.conheight-SBAR_HEIGHT, num);
}
...
void Sbar_DrawString (int x, int y, char *str)
{
	if (cl.gametype == GAME_DEATHMATCH)
		...
		Draw_String (x + ((vid.conwidth - CON_SCREEN_WIDTH)>>1), y+ vid.conheight-SBAR_HEIGHT, str);
}
...
// weapons
	for (i=0 ; i<7 ; i++)
	{
		if (cl.items & (IT_SHOTGUN<<i) )
		{
         ...
         //Sbar_DrawPic (8,(i*8) -(vid.conheight-SBAR_HEIGHT)+8, sb_weapons[flashon][i]);
		...
		for (i=0 ; i<4 ; i++)
		{
			if (cl.items & (1<<(28+i)))
			{
				...
				{	// flash frame
					...
					Sbar_DrawPic (CON_SCREEN_WIDTH-32 + i*8, -4, sb_sigil[i]);
				if (time &&	time > cl.time - 2)
					sb_updates = 0;
			}
		}
...
// draw the text
	...
	if (cl.gametype == GAME_DEATHMATCH)
		...
		xofs = (vid.conwidth - CON_SCREEN_WIDTH)>>1;
...
// PGM 03/02/97 - fixed so color swatch only appears in CTF modes
        ...
        (teamplay.value<7))
	{
		...
		if (cl.gametype == GAME_DEATHMATCH)
			...
			xofs = ((vid.conwidth - CON_SCREEN_WIDTH)>>1) + 113;
...
// PGM 01/19/97 - team color drawing
	...
	{
		Sbar_DrawPic (104, 0, sb_face_invis_invuln);
		return;
	}
	if (cl.items & IT_QUAD)
	{
		Sbar_DrawPic (104, 0, sb_face_quad );
		return;
	}
	if (cl.items & IT_INVISIBILITY)
	{
		Sbar_DrawPic (104, 0, sb_face_invis );
		return;
	}
	if (cl.items & IT_INVULNERABILITY)
	{
		Sbar_DrawPic (104, 0, sb_face_invuln);
		return;
	}
...
void Sbar_Draw (void)
{
	...
	//if (sb_lines && vid.conwidth > CON_SCREEN_WIDTH) 
		...
		else
		{
			...
			else
			{
				...
				if (cl.items & IT_ARMOR3)
					Sbar_DrawPic (8, 0, sb_armor[2]);
				else if (cl.items & IT_ARMOR2)
					Sbar_DrawPic (8, 0, sb_armor[1]);
				else if (cl.items & IT_ARMOR1)
					Sbar_DrawPic (8, 0, sb_armor[0]);
			}
		// health
			Sbar_DrawNum (32, 0, cl.stats[STAT_HEALTH], 3
	...
	// ammo icon
		...
		else
		{
			if (cl.items & IT_SHELLS)
				Sbar_DrawPic (132, 0, sb_ammo[0]);
			else if (cl.items & IT_NAILS)
				Sbar_DrawPic (132, 0, sb_ammo[1]);
			else if (cl.items & IT_ROCKETS)
				Sbar_DrawPic (132, 0, sb_ammo[2]);
			else if (cl.items & IT_CELLS)
				Sbar_DrawPic (132, 0, sb_ammo[3]);
		}

		Sbar_DrawNum (152, 0, cl.stats[STAT_AMMO], 3,
					  cl.stats[STAT_AMMO] <= 10);
	}

	if (vid.conwidth > CON_SCREEN_WIDTH) {
		if (cl.gametype == GAME_DEATHMATCH)
...
void Sbar_DeathmatchOverlay (void)
{
	...
	M_DrawPic ((CON_SCREEN_WIDTH-pic->width)/2, 8, pic);
...
// draw the text
	...
	x = 80 + ((vid.conwidth - CON_SCREEN_WIDTH)>>1);
	y = 40;
	for (i=0 ; i<l ; i++)
	{
		k = fragsort[i];
