...
void SV_SendServerinfo (client_t *client)
{
	...
	sprintf (message, pr_strings+sv.edicts[0]->v.message);
...
// allocate server memory
	...
	//the ent alloc sequence must be done in this order
// leave slots at start for clients only
	//sv.num_edicts = svs.maxclients+1;
	//aloc world edict - put here so it will be first
	ent = ED_Alloc();
